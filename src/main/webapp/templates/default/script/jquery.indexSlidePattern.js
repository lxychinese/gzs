(function ($) {
	$.fn.extend({
		IMGDEMO: function (opt) {
			...
			var $button = $(this).children(".child");			//容器;
		...
		anim: function () {										//动画方法;
			var $fx = function () {
				$(this).siblings(".child").animate({
					width: $default.minWidth
				}, $default.speed).removeClass('on');
				$(this).animate({
					width: $default.maxWidth
				}, $default.speed).addClass('on');

				//$(this).siblings("li").children("div").fadeOut();
				//$(this).child("div").fadeTo($default.speed, 0.7);
				$(this).dequeue();
			};
			$(this).queue($fx);
		}
